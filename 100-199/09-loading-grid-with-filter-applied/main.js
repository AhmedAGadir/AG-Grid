var columnDefs = [
    { field: 'country', filter: 'agSetColumnFilter', filterParams: { values: countries(), newRowsAction: 'keep' } },
...
function saveState() {
    ...
    pivotState = gridOptions.columnApi.isPivotMode();
    console.log('column state saved');

    //this is a simulation of storing a filter state into the databasde
    localStorage.setItem('filterState', JSON.stringify(gridOptions.api.getFilterModel()));
}

function getFilterStateFromServer() {
    //simulation of restoring the filter state from the database
    //note that the grid will load everything and after 3 seconds it will reload again with the filter applied
    setTimeout(function () {
        filterState = JSON.parse(localStorage.getItem('filterState'));
        console.log(':::DZ::: filterState: ', filterState);
        restoreState('filter');
    }, 3000);
}
...
function restoreState(type) {
    ...
    if (type && type === 'filter') {
        if (filterState) gridOptions.api.setFilterModel(filterState);
    } else {
        ...
        if (filterState) gridOptions.api.setFilterModel(filterState);
        gridOptions.columnApi.setPivotMode(pivotState);
    }
...
function printState() {
    ...
    var pivotState = gridOptions.columnApi.isPivotMode();
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    agGrid
        ...
        .then(function (data) {
            ...
            // let invisibleGrid = document.createElement('div');
            // new agGrid.Grid(invisibleGrid, invisibleGridOptions);
            // let invisibleDataSource = createServerSideDatasource(fakeServer, invisibleGridOptions);
            // invisibleGridOptions.api.setServerSideDatasource(invisibleDataSource);
            // let filteredRows = [];
            // invisibleGridOptions.api.forEachNode(node => filteredRows.push(node));
            // setTimeout(() => console.log(filteredRows), 3000);

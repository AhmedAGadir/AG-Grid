...
function onRowDragEnd(params) {
    ...
    // filter out moving node and all its children
    let updatedRows = store.rowData.filter(row => movingNode.data.filePath.some((dir, ind) => row.filePath[ind] !== dir))

    // generate new paths for moving node and its children
    let rowsToAdd = [];
    generateRowsToAdd(overNode.data.filePath, overNode.data.type, params.node, rowsToAdd);

...
let imgSettler = debounce(
    ()=> {      
      console.log (`all images are loaded now`, nodesToSettle)
...
var columnDefs = [
  ...
  { 
    ...
    cellRendererParams: {
      onImageLoaded: (node, imgHeight) => { 
        console.log (`onImageLoaded: rowNode ${node.id}: height: ${imgHeight}px`)
...
var gridOptions = {
  ...
  getRowHeight: (params)=> {
    ...
    console.log (`getRowHeight: rowNode ${params.node.id}: height: ${rowHeight}px`);
    return rowHeight;
  }
};

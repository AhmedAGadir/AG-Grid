...
var gridOptions = {
    ...
    defaultColDef: {
        ...
        suppressKeyboardEvent: params => {
            console.log(params.editing, event.key)
            if (params.editing && event.key === 'ArrowUp' || params.editing && event.key === 'ArrowDown') {
...
MyCellEditor.prototype.processKeyboardEvent = function (event) {
    let deltaY = event.key === 'ArrowUp' ? -1 : 1;
        this.params.api.setFocusedCell(
            this.params.rowIndex + deltaY,
            this.params.column
        );
        this.params.api.startEditingCell({
            rowIndex: this.params.rowIndex + deltaY,

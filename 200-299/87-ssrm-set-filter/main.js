...
function updateVals(data) {
    ...
    data.forEach(row => {
        athleteSet.add(row.athlete);
        countrySet.add(row.country);
    });
    vals.athlete = [...athleteSet];
    vals.country = [...countrySet];
}
...
FakeServer.prototype.filterList = function(data, filterModel) {
    ...
    for (var i = 0; i<data.length; i++) {
        ...
        if (filterModel.athlete) {
            if (filterModel.athlete.values.indexOf(item.athlete)<0) {
                continue;
            }
        }
